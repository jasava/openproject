...
RSpec.describe "Cards requests", :skip_csrf, type: :rails_request do
  ...
  shared_let(:editor) do
    create(:user,
           member_with_permissions: { project => %i[view_community_team_planner view_work_packages
                                                    edit_work_packages] })
  end
  ...
  let(:schedule) { create(:team_schedule, project:, principal: editor, public: true) }
  let(:work_package) do
    create(:work_package, project:, assigned_to: editor, start_date: Date.new(2026, 8, 3), due_date: Date.new(2026, 8, 5))
  end

  subject(:perform) do
    put card_project_team_schedule_path(project, schedule, work_package_id: work_package.id),
  ...
  describe "PUT update" do
    ...
    it "404s for a work package outside the current user's visible scope" do
      ...
      login_as editor
      put card_project_team_schedule_path(project, schedule, work_package_id: foreign_wp.id),
    ...
    it "ignores unpermitted params (mass-assignment protection)" do
      login_as editor

      put card_project_team_schedule_path(project, schedule, work_package_id: work_package.id),
          params: { start_date: "2026-08-04", project_id: 999_999, type_id: 999_999 }, as: :turbo_stream
