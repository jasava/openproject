...
// which only reorders/reparents DOM nodes — see
// docs/community-team-planner/architecture-analysis.md), so this is a small,
...
export default class CardController extends Controller {
  ...
  connect() {
    this.startHandle = this.element.querySelector('[data-team-schedule--card-target="startHandle"]');
    this.endHandle = this.element.querySelector('[data-team-schedule--card-target="endHandle"]');
  ...
  private beginDrag(event:PointerEvent, mode:'move'|'resize-start'|'resize-end') {
    ...
    document.body.setAttribute('data-team-schedule-dragging', 'true');
  }
  ...
  private endDrag(target:HTMLElement, pointerId:number) {
    this.dragging = false;
    document.body.removeAttribute('data-team-schedule-dragging');
  ...
  private async submit(originCell:HTMLElement, targetCell:HTMLElement) {
    ...
    const surface = (this.element as HTMLElement).closest<HTMLElement>('.op-team-schedule--surface');
  ...
  private dayCellAt(x:number, y:number):HTMLElement|null {
    return document.elementsFromPoint(x, y)
      .find((el):el is HTMLElement => el instanceof HTMLElement && el.classList.contains('op-team-schedule--day-cell'))
      ?? null;
  }
